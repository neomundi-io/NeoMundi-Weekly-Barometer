...
def render_points(spec: dict, profiles: list[dict], marker_id: str = "arrOutlier") -> tuple[str, str]:
    """
    Retourne (points_dans_cadre, rail_des_profils_au_dela_de_l_echelle).

    L'échelle principale reste limitée pour garder lisibles les profils groupés
    à gauche. Les profils qui dépassent cette limite sont affichés dans un rail
    latéral lisible, sans chevauchement ni flèches longues.
    """
    C = spec["colors"]
    ax = spec["axes"]["x"]
    ay = spec["axes"]["y"]
    ...
    for p in profiles:
        ...
        if is_x_outlier(spec, fact):
            ...
            actual_cy = max(can["plot_y_max_px"] + 6, min(can["plot_y_min_px"] - 6, actual_cy))
            outliers.append({
                "pid": pid,
                "fact": fact,
                "sem": sem,
  ...
  <circle class="pt" cx="{cx}" cy="{cy}" r="6" fill="{C['point_fill']}"
    ...
    if outliers:
        ...
        cutoff = f"{ax['domain_max']:.1f}".replace(".", ",")
  ...
  <text x="{rail_x}" y="{rail_top - 18}" font-size="12" fill="{C['annotation_text']}" font-weight="600">
    Au-delà de l’échelle principale
  </text>
  <text x="{rail_x}" y="{rail_top - 4}" font-size="10.5" fill="{C['annotation_text']}">
    plus de {cutoff} % de réponses à vérifier
  </text>""")
  ...
  <text x="{rail_x + 80}" y="{card_y + 35}" font-size="10.5"
        fill="{C['outlier_badge_text']}">réponses à vérifier</text>
  <text x="{rail_x + 16}" y="{card_y + 52}" font-size="10.5"
        fill="{C['outlier_badge_text']}">variation du sens : {item['sem_fmt']} %</text>""")
