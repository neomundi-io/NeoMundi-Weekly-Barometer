...
def render_points(spec: dict, profiles: list[dict]) -> tuple[str, str]:
  ...
  <text x="{arrow_cx + 3}" y="{arrow_y2 - 18}" text-anchor="middle"
        font-size="10" fill="{C['outlier_badge_text']}">Risque factuel</text>
...
<title>NeoMundi Baromètre #{week} — Cartographie</title>
<style>
...
<div class="wrap" id="cartoWrap_{uid}">
  <p class="meta">Baromètre NeoMundi · semaine {week} · {n_profiles} profils dé-identifiés · généré le {generated}</p>
  <h2 class="sr-only">Cartographie de {n_profiles} profils dé-identifiés. Axe horizontal : risque factuel, signal mesuré non assimilable à une réponse fausse. Axe vertical : variation sémantique, degré auquel le sens des réponses change d'une fois à l'autre. Spécification graphique figée — comparable avec toutes les semaines.</h2>
  <svg viewBox="0 0 680 492" xmlns="http://www.w3.org/2000/svg" width="100%"
       role="img" aria-label="Nuage de {n_profiles} profils — Baromètre NeoMundi semaine {week}">
    <defs>
...
<!-- spec_version: {spec_version} | chart_spec.json figé -->
</body>
</html>
"""
...
def generate(spec: dict, profiles: list[dict], week: int, output_path: Path) -> None:
    ...
    if outliers_x:
        for p in outliers_x:
            print(f"[OUTLIER-X] {p['profile_id']} — risque factuel {p['factual_risk_pct']:.2f} % "
                  f"(max échelle : {ax['domain_max']} %) → flèche hors cadre.")
    if outliers_y:
        for p in outliers_y:
            print(f"[OUTLIER-Y] {p['profile_id']} — variation sémantique {p['semantic_variation_pct']:.2f} % "
